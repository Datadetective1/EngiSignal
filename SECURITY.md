...
- One identity is not a signed-in customer: the ingestion worker, which writes large imports on the database's schedule after the uploading request has ended. It could not use RLS, because it has no session to borrow — a token expires and a browser closes, while an import must survive a deploy, a crash and a retry an hour later. Rather than the service-role key this ruled out, it authenticates as `ingestion_worker`: a database role with `EXECUTE` on six job functions and no rights over any table in any schema. Asserted against production rather than assumed — the role's reads of `imports` and `ingestion_usage` are refused by Postgres. Its only table privilege anywhere is `select` on `storage.objects`, confined by policy to one bucket. Every function it can call takes an import id and derives the organization from the import row, so reaching another tenant is not a permission it is denied but a request it has no way to phrase, and a test asserts that no call it makes ever names an organization.
- `INGESTION_WORKER_TOKEN` is minted out of band by `scripts/mint-ingestion-worker-token.mjs`. The project JWT secret that signs it can mint a token for any role, including `service_role`, and is deliberately never present in the deployment environment — only the narrow token it produced.
